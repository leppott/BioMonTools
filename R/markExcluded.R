...
#'
#' @description Takes as an input data frame with Sample ID, Taxa ID, and phlogenetic name fields
...
#' These taxa are removed from the taxa richness metrics.  This is because these are coarser level taxa.
#'
...
#' a practical case.
#'
#' Taxa Levels are phylogenetic names that are to be checked.  They should be listed in order
#' from course (kingdom) to fine (species).  Names not appearing in the data will be skipped.
#'
...
#' @param SampID Column name in df_samptax for sample identifier.  Default = "SAMPLEID".
#' @param TaxaID Column name in df_samptax for organism identifier.  Default = "TAXAID".
#' @param TaxaCount Column name in df_samptax for organism count.  Default = "N_TAXA".
#' @param Exclude Column name for Excluded Taxa results in returned data frame.  Default = "Excluded".
#' @param TaxaLevels Column names in df_samptax that for phylogenetic names to be evaluated.
#' Need to be in order from coarse to fine (i.e., Phylum to Species).
#' @param Exceptions NA or two column data frame of synonyms or other exceptions.
...
#'
#' @return Returns a data frame of df_samptax with an additional column, ExcludedTaxa.
#'
#' @examples
#'
#' # Packages
...
#
#' @export
markExcluded <- function(df_samptax, SampID="SAMPLEID", TaxaID="TAXAID", TaxaCount = "N_TAXA"
                         , Exclude="EXCLUDED", TaxaLevels, Exceptions){##FUNCTION.markExcluded.START
  #
  boo_QC <- FALSE
  ...
  if (sum(Exclude %in% names(df_samptax))==1 & interactive()==TRUE){##IF.prompt.START
    #ask to continue
  ...
  for (i in tl_present){##for.i.START
    ...
    if(sum(!is.na(Exceptions))!=0){##IF.is.na.START
      ...
      for (a in Exceptions[,2]){##FOR.a.START
        ...
        df_samptax[, i] <- ifelse(df_samptax[,i]==a, as.character(Exceptions[,1][a.num]), df_samptax[,i])
      }##FOR.a.END
    }##IF.is.na.END
    ...
    # i_count_orig <- dplyr::summarise(dplyr::group_by(df_samptax, SampleID, Phylum)
    #               # individuals #
    ...
    # i_not_unique <- dplyr::summarise_se(seplyr::group_by_se(df_samptax, SampID, i)
    #               # individuals #
    ...
    #                                 , by=c("SampleID"="SampleID", "TaxaID"="Order"))


    # left_join(df_Exclude, i_count_dups, by=c("SampleID"="SampleID", "TaxaID"= "Phylum"))
    #
    ...
    #left_join(df_Exclude, i_count_dups, by=c("SampleID"="SampleID", "TaxaID"= as_character(i)))
    ...
    by_var <- c(stats::setNames(nm=by_x_1, by_y_1), stats::setNames(nm=by_x_2, by_y_2))
